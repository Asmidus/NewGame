...
void AABBTree::insertObject(entt::entity object)
{
	unsigned nodeIndex = allocateNode();
	AABBNode& node = _nodes[nodeIndex];
	node.aabb = AABB(object);
...
void AABBTree::removeObject(entt::entity object)
{
...
void AABBTree::updateObject(entt::entity object)
{
	unsigned nodeIndex = _objectNodeIndexMap[object];
	//updateLeaf(nodeIndex, object->getAABB());
	updateLeaf(nodeIndex, AABB(object));
}

std::forward_list<entt::entity> AABBTree::queryOverlaps(entt::entity object) const
{
	std::forward_list<entt::entity> overlaps;
